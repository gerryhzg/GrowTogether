...
export function StreakCard({ streakInfo }: StreakCardProps) {
  ...
  return (
    <div className="rounded-[1.5rem] bg-white/75 p-5 shadow-sm">
      <p className="text-xs uppercase tracking-[0.25em] text-muted">{isNeonQuest ? "Streak Meter" : isWoodland ? "Steady Growth" : isFarm ? "Steady as Sunrise" : "Consistency Tracker"}</p>
