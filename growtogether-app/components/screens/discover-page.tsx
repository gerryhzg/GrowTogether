...
export function DiscoverPage() {
  ...
  async function handleGenerateGoals(event: FormEvent<HTMLFormElement>) {
    ...
    setSaved(true);
    try {
      const response = await fetch("/api/ai/goals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ interests: interestPayload }) });
  ...
  async function handleStartJourney() {
    ...
    await createJourney({ goalTitle: customTitle || selectedGoal.title, goalDescription: customDescription || selectedGoal.description, targetCount: customTargetCount, unit: customUnit, linkedInterest: selectedGoal.linkedInterest });
  }
  ...
  async function loadExplanation(index: number) {
    ...
    try {
      const response = await fetch("/api/ai/goal-explanation", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ goal, interests: INTEREST_OPTIONS.map((i) => ({ interest: i, rating: ratings[i] })) }) });
      ...
      <Panel>
        ...
        {isChild && journey && (
          <div className={`mt-4 rounded-2xl px-4 py-3 text-sm font-medium ${journey.approved_by_parent ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"}`}>
            {journey.approved_by_parent ? journey.parent_override ? "✏️ Your parent changed your goal! Check it out!" : "✅ Your parent approved your goal! Let's go!" : "⏳ Waiting for your parent to approve your goal..."}
          </div>
        )}
        ...
        {goalSuggestions.length > 0 && (
          <div className="mt-8 space-y-4 rounded-[1.5rem] bg-white/75 p-5 shadow-sm">
            ...
            <button type="button" disabled={!customTitle.trim()} onClick={handleStartJourney} className="rounded-full bg-secondary px-5 py-3 text-sm font-semibold text-white transition hover:bg-secondary/90 disabled:opacity-50">{isChild ? "Start my journey! 🌟" : "Start this growth journey"}</button>
          </div>
        )}
      </Panel>
    </div>
